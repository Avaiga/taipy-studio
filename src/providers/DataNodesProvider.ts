...
export class DataNodesProvider implements TreeDataProvider<DataNodeItem> {
  private _onDidChangeTreeData: EventEmitter<DataNodeItem | undefined> =
    new EventEmitter<DataNodeItem | undefined>();
  readonly onDidChangeTreeData: Event<DataNodeItem | undefined> =
  ...
  async refresh(context: Context): Promise<void> {
    ...
    commands.executeCommand(
      ...
      dataNodeEntries.length
    );
    const dataNodes: DataNodeItem[] = dataNodeEntries.map(
      (entry) => new DataNodeItem(entry[0], entry)
    );
