...
export const reportInconsistencies = (doc: TextDocument, toml: JsonMap) => {
  ...
  [DataNode, Task, Pipeline].forEach(nodeType => toml[nodeType] && Object.entries(toml[nodeType]).filter(([nodeName, _]) => "default" != nodeName && !nodeIds.has(`${nodeType}.${nodeName}`)).forEach(([nodeName, element]) => {
    // @ts-ignore
